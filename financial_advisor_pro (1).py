...
def show_stock_analysis():
    ...
    stock_investment = st.number_input(
        ...
        min_value=1000,
        max_value=None,
        value=int(recommended_monthly_investment * 0.4),
        step=1000,
        help="Recommended: 40% of your investment capacity for stocks"
    )
    
    # Investment strategy based on amount
    if stock_investment > recommended_monthly_investment * 0.4:
        st.warning("""
    ...
    with tab2:
        ...
        if market_sentiments:
            ...
            fig = px.treemap(
                sentiment_df,
                path=['Sector'],
                values=abs(sentiment_df['Sentiment']),
                color='Sentiment',
            ...
            if search_results:
                ...
                with st.spinner("Analyzing market sentiment..."):
                    ...
                    if sentiment_data:
                        ...
                        with col2:
                            positive_ratio = sentiment_data['positive_count'] / (sentiment_data['positive_count'] + sentiment_data['negative_count'] + sentiment_data['neutral_count'])
                            st.metric("Positive News Ratio", f"{positive_ratio:.1%}")
                        
                        with col3:
                            news_count = len(sentiment_data['news_items'])
...
def show_predictions(data, stock_name):
    """Display stock predictions using different models"""
    ...
    try:
        ...
        with st.spinner("Training LSTM model..."):
            X_train, X_test, y_train, y_test, scaler = prepare_lstm_data(prices, lookback)
            ...
            # Calculate accuracy
            lstm_accuracy = calculate_prediction_accuracy(actual_test.flatten(), lstm_pred.flatten())
        ...
        # Mean Prediction
        mean_pred = np.full(prediction_days, prices.mean())
        mean_accuracy = calculate_prediction_accuracy(
            prices[-prediction_days:],
            mean_pred[:len(prices[-prediction_days:])]
        )
        ...
        metrics_df = pd.DataFrame({
            'LSTM': {
                'MSE': float(lstm_accuracy['MSE']),
                'RMSE': float(lstm_accuracy['RMSE']),
                'MAE': float(lstm_accuracy['MAE']),
                'MAPE': float(lstm_accuracy['MAPE'])
            },
            'QMC': {
                'MSE': float(qmc_accuracy['MSE']),
                'RMSE': float(qmc_accuracy['RMSE']),
                'MAE': float(qmc_accuracy['MAE']),
                'MAPE': float(qmc_accuracy['MAPE'])
            },
            'Mean': {
                'MSE': float(mean_accuracy['MSE']),
                'RMSE': float(mean_accuracy['RMSE']),
                'MAE': float(mean_accuracy['MAE']),
                'MAPE': float(mean_accuracy['MAPE'])
            }
        }).round(2)
        ...
        summary_data = pd.DataFrame({
            'Model': ['LSTM', 'QMC', 'Mean'],
            'Current Price': [prices[-1], prices[-1], prices[-1]],
            'Predicted Price': [float(lstm_pred[-1]), float(qmc_mean[-1]), float(mean_pred[-1])],
            'Change (%)': [lstm_change, qmc_change, 0.0]
        })
